(define (problem depotprob5646) (:domain Depot)
(:objects
	depot0 depot1 depot2 distributor0 distributor1 distributor2 truck0 truck1 pallet0 pallet1 pallet2 pallet3 pallet4 pallet5 pallet6 pallet7 pallet8 pallet9 crate0 crate1 crate2 crate3 crate4 crate5 hoist0 hoist1 hoist2 hoist3 hoist4 hoist5 )
(:init
	(pallet pallet0)
	(surface pallet0)
	(at pallet0 depot0)
	(clear crate2)
	(pallet pallet1)
	(surface pallet1)
	(at pallet1 depot1)
	(clear pallet1)
	(pallet pallet2)
	(surface pallet2)
	(at pallet2 depot2)
	(clear crate5)
	(pallet pallet3)
	(surface pallet3)
	(at pallet3 distributor0)
	(clear crate4)
	(pallet pallet4)
	(surface pallet4)
	(at pallet4 distributor1)
	(clear pallet4)
	(pallet pallet5)
	(surface pallet5)
	(at pallet5 distributor2)
	(clear pallet5)
	(pallet pallet6)
	(surface pallet6)
	(at pallet6 distributor1)
	(clear pallet6)
	(pallet pallet7)
	(surface pallet7)
	(at pallet7 depot0)
	(clear pallet7)
	(pallet pallet8)
	(surface pallet8)
	(at pallet8 depot0)
	(clear crate3)
	(pallet pallet9)
	(surface pallet9)
	(at pallet9 distributor0)
	(clear pallet9)
	(truck truck0)
	(at truck0 distributor1)
	(truck truck1)
	(at truck1 depot0)
	(hoist hoist0)
	(at hoist0 depot0)
	(available hoist0)
	(hoist hoist1)
	(at hoist1 depot1)
	(available hoist1)
	(hoist hoist2)
	(at hoist2 depot2)
	(available hoist2)
	(hoist hoist3)
	(at hoist3 distributor0)
	(available hoist3)
	(hoist hoist4)
	(at hoist4 distributor1)
	(available hoist4)
	(hoist hoist5)
	(at hoist5 distributor2)
	(available hoist5)
	(crate crate0)
	(surface crate0)
	(at crate0 depot2)
	(on crate0 pallet2)
	(crate crate1)
	(surface crate1)
	(at crate1 depot2)
	(on crate1 crate0)
	(crate crate2)
	(surface crate2)
	(at crate2 depot0)
	(on crate2 pallet0)
	(crate crate3)
	(surface crate3)
	(at crate3 depot0)
	(on crate3 pallet8)
	(crate crate4)
	(surface crate4)
	(at crate4 distributor0)
	(on crate4 pallet3)
	(crate crate5)
	(surface crate5)
	(at crate5 depot2)
	(on crate5 crate1)
	(place depot0)
	(place depot1)
	(place depot2)
	(place distributor0)
	(place distributor1)
	(place distributor2)
)
 (:utility 
    (= (on crate0 pallet0) 10) 
    (= (on crate1 pallet5) 10) 
    (= (on crate2 pallet4) 10) 
    (= (on crate3 pallet7) 10) 
    (= (on crate4 pallet9) 10) 
    (= (on crate5 pallet1) 10) 
    (= (on crate3 pallet1) 2) 
    (= (on crate0 pallet4) 3) 
    (= (lifting hoist2 crate5) 1) 
    (= (at crate1 distributor0) 2) 
    (= (lifting hoist1 crate0) 5) 
    (= (lifting hoist0 crate2) 2) 
    (= (lifting hoist0 crate3) 4) 
    (= (on crate4 pallet7) 3) 
    (= (on crate3 pallet6) 1) 
    (= (on crate5 pallet7) 5) 
    (= (lifting hoist3 crate2) 4) 
 )
 (:bound 25)
 )
 



