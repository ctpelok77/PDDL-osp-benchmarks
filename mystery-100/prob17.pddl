(define (problem strips-mysty-x-17)
   (:domain mystery-strips)
   (:objects papaya endive cherry rice wurst yogurt potato onion
             marzipan shrimp wonderbread mutton turkey arugula ham haroset
             excitement achievement satiety love entertainment satisfaction
             lubricity intoxication rest dread hangover grief depression
             laceration jealousy prostatitis abrasion loneliness angina anger
             anxiety boils jealousy-2 sciatica boils-1 loneliness-3
             abrasion-4 depression-7 laceration-8 goias bavaria guanabara
             bosnia alsace surrey mercury saturn earth uranus)
   (:init (food papaya)
          (food endive)
          (food cherry)
          (food rice)
          (food wurst)
          (food yogurt)
          (food potato)
          (food onion)
          (food marzipan)
          (food shrimp)
          (food wonderbread)
          (food mutton)
          (food turkey)
          (food arugula)
          (food ham)
          (food haroset)
          (pleasure excitement)
          (pleasure achievement)
          (pleasure satiety)
          (pleasure love)
          (pleasure entertainment)
          (pleasure satisfaction)
          (pleasure lubricity)
          (pleasure intoxication)
          (pleasure rest)
          (pain dread)
          (pain hangover)
          (pain grief)
          (pain depression)
          (pain laceration)
          (pain jealousy)
          (pain prostatitis)
          (pain abrasion)
          (pain loneliness)
          (pain angina)
          (pain anger)
          (pain anxiety)
          (pain boils)
          (pain jealousy-2)
          (pain sciatica)
          (pain boils-1)
          (pain loneliness-3)
          (pain abrasion-4)
          (pain depression-7)
          (pain laceration-8)
          (province goias)
          (province bavaria)
          (province guanabara)
          (province bosnia)
          (province alsace)
          (province surrey)
          (planet mercury)
          (planet saturn)
          (planet earth)
          (planet uranus)
          (craves anger marzipan)
          (locale shrimp alsace)
          (craves prostatitis wurst)
          (locale yogurt bosnia)
          (locale arugula surrey)
          (eats papaya rice)
          (eats potato yogurt)
          (locale papaya alsace)
          (orbits saturn earth)
          (attacks goias bavaria)
          (harmony satisfaction saturn)
          (eats turkey haroset)
          (eats endive wurst)
          (craves lubricity shrimp)
          (eats onion ham)
          (attacks bavaria guanabara)
          (craves boils-1 arugula)
          (orbits mercury saturn)
          (craves abrasion yogurt)
          (eats ham onion)
          (craves dread papaya)
          (eats papaya wurst)
          (eats wonderbread yogurt)
          (eats wurst papaya)
          (eats rice papaya)
          (eats potato shrimp)
          (locale marzipan surrey)
          (locale wonderbread alsace)
          (eats yogurt potato)
          (eats wonderbread ham)
          (eats marzipan rice)
          (craves sciatica wonderbread)
          (eats potato arugula)
          (harmony rest saturn)
          (craves intoxication arugula)
          (eats marzipan cherry)
          (eats endive cherry)
          (craves laceration cherry)
          (eats marzipan haroset)
          (eats ham mutton)
          (eats marzipan onion)
          (locale potato bosnia)
          (eats shrimp potato)
          (craves angina onion)
          (eats shrimp onion)
          (locale mutton guanabara)
          (locale ham guanabara)
          (craves love rice)
          (eats cherry rice)
          (harmony lubricity uranus)
          (eats rice cherry)
          (craves grief endive)
          (locale onion alsace)
          (craves depression endive)
          (eats mutton ham)
          (eats wurst endive)
          (craves satisfaction potato)
          (harmony entertainment uranus)
          (eats ham wonderbread)
          (eats yogurt wonderbread)
          (craves abrasion-4 ham)
          (eats cherry marzipan)
          (locale wurst goias)
          (locale cherry alsace)
          (harmony excitement uranus)
          (harmony achievement earth)
          (locale endive alsace)
          (eats marzipan mutton)
          (craves loneliness-3 ham)
          (craves jealousy-2 wonderbread)
          (craves achievement endive)
          (attacks guanabara bosnia)
          (eats haroset turkey)
          (harmony love uranus)
          (harmony intoxication uranus)
          (eats arugula potato)
          (locale haroset guanabara)
          (eats shrimp turkey)
          (eats haroset marzipan)
          (craves satiety cherry)
          (orbits earth uranus)
          (eats rice marzipan)
          (craves entertainment wurst)
          (eats wonderbread arugula)
          (eats turkey shrimp)
          (locale turkey bavaria)
          (harmony satiety saturn)
          (craves loneliness potato)
          (craves anxiety marzipan)
          (craves excitement papaya)
          (eats onion shrimp)
          (eats arugula wonderbread)
          (craves laceration-8 haroset)
          (craves boils shrimp)
          (craves rest ham)
          (attacks bosnia alsace)
          (locale rice alsace)
          (eats onion marzipan)
          (attacks alsace surrey)
          (craves depression-7 haroset)
          (eats cherry endive)
          (craves jealousy rice)
          (eats mutton marzipan)
          (craves hangover papaya))
 (:utility 
    (= (craves abrasion wonderbread) 10) 
    (= (craves anger marzipan) 5) 
    (= (craves lubricity arugula) 5) 
    (= (craves angina mutton) 2) 
    (= (fears hangover love) 4) 
    (= (fears prostatitis intoxication) 5) 
    (= (fears laceration love) 4) 
    (= (craves sciatica onion) 3) 
    (= (fears grief lubricity) 2) 
    (= (craves intoxication yogurt) 1) 
    (= (craves sciatica turkey) 5) 
    (= (craves jealousy wonderbread) 4) 
    (= (craves boils-1 cherry) 4) 
    (= (fears boils-1 satisfaction) 5) 
    (= (fears anger excitement) 1) 
    (= (craves abrasion-4 rice) 5) 
    (= (fears jealousy-2 achievement) 1) 
    (= (craves sciatica haroset) 3) 
    (= (harmony satiety earth) 2) 
    (= (craves love endive) 2) 
    (= (locale marzipan goias) 1) 
    (= (locale potato goias) 3) 
    (= (craves loneliness-3 cherry) 5) 
    (= (craves satiety wurst) 4) 
    (= (craves satisfaction arugula) 3) 
    (= (craves intoxication wurst) 2) 
    (= (locale papaya bosnia) 3) 
    (= (craves achievement ham) 2) 
    (= (harmony satiety uranus) 2) 
    (= (craves rest turkey) 4) 
    (= (craves abrasion-4 haroset) 2) 
    (= (craves prostatitis mutton) 1) 
    (= (craves abrasion rice) 3) 
    (= (craves depression cherry) 1) 
    (= (craves loneliness marzipan) 1) 
    (= (craves depression-7 yogurt) 4) 
    (= (fears dread entertainment) 5) 
 )
 (:bound 4)
 )
 


   