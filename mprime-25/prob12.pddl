(define (problem strips-mprime-x-12)
   (:domain mystery-prime-strips)
   (:objects broccoli turkey lettuce kale hotdog pork wurst arugula
             excitement intoxication love rest abrasion grief anger kentucky
             pennsylvania quebec arizona surrey guanabara goias pluto jupiter
             neptune)
   (:init (food broccoli)
          (food turkey)
          (food lettuce)
          (food kale)
          (food hotdog)
          (food pork)
          (food wurst)
          (food arugula)
          (pleasure excitement)
          (pleasure intoxication)
          (pleasure love)
          (pleasure rest)
          (pain abrasion)
          (pain grief)
          (pain anger)
          (province kentucky)
          (province pennsylvania)
          (province quebec)
          (province arizona)
          (province surrey)
          (province guanabara)
          (province goias)
          (planet pluto)
          (planet jupiter)
          (planet neptune)
          (eats lettuce arugula)
          (locale wurst goias)
          (attacks surrey guanabara)
          (locale pork pennsylvania)
          (locale lettuce surrey)
          (eats hotdog kale)
          (attacks guanabara goias)
          (craves abrasion hotdog)
          (eats wurst broccoli)
          (harmony rest jupiter)
          (attacks quebec arizona)
          (craves excitement broccoli)
          (harmony intoxication jupiter)
          (eats broccoli wurst)
          (eats arugula wurst)
          (eats turkey broccoli)
          (craves love hotdog)
          (eats pork wurst)
          (orbits pluto jupiter)
          (eats hotdog lettuce)
          (craves anger arugula)
          (eats kale pork)
          (eats lettuce wurst)
          (harmony love jupiter)
          (craves grief hotdog)
          (craves rest arugula)
          (locale broccoli quebec)
          (eats lettuce hotdog)
          (locale turkey arizona)
          (eats kale hotdog)
          (locale kale pennsylvania)
          (craves intoxication lettuce)
          (attacks pennsylvania quebec)
          (eats wurst lettuce)
          (eats broccoli turkey)
          (eats pork kale)
          (eats wurst arugula)
          (attacks kentucky pennsylvania)
          (harmony excitement neptune)
          (orbits jupiter neptune)
          (eats turkey hotdog)
          (eats arugula lettuce)
          (locale hotdog pennsylvania)
          (locale arugula kentucky)
          (eats wurst pork)
          (eats hotdog turkey)
          (attacks arizona surrey))
 (:utility 
    (= (craves anger kale) 10) 
    (= (craves excitement arugula) 5) 
    (= (locale kale pennsylvania) 1) 
    (= (harmony love pluto) 3) 
    (= (locale broccoli surrey) 3) 
    (= (locale lettuce kentucky) 2) 
    (= (locale hotdog arizona) 4) 
 )
 (:bound 1)
 )
 


   
