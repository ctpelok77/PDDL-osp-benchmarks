(define (problem strips-mprime-x-5)
   ...
   (:init (food broccoli)
          ...
          (eats turkey tuna))
 (:utility 
    (= (craves loneliness shrimp) 10) 
    (= (craves grief shrimp) 10) 
    (= (locale chocolate goias) 1) 
    (= (fears boils excitement) 5) 
    (= (fears abrasion lubricity) 2) 
    (= (craves sciatica scallop) 4) 
    (= (craves satisfaction scallop) 4) 
    (= (craves jealousy turkey) 3) 
    (= (craves anger scallop) 3) 
    (= (fears prostatitis excitement) 5) 
    (= (locale broccoli alsace) 1) 
    (= (fears laceration satisfaction) 3) 
    (= (craves satisfaction tuna) 1) 
    (= (craves grief tuna) 5) 
 )
 (:bound 8)
 )
