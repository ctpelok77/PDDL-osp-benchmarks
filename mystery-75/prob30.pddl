(define (problem strips-mysty-x-30)
 ...
 (:utility 
    ...
    (= (locale turkey pennsylvania) 4) 
    (= (fears anxiety achievement) 3) 
    (= (craves expectation yogurt) 1) 
    (= (craves sciatica beef) 1) 
    (= (craves achievement yogurt) 4) 
    (= (craves abrasion hamburger) 1) 
    (= (craves excitement hamburger) 5) 
    (= (craves triumph muffin) 4) 
    (= (locale guava arizona) 2) 
    (= (fears prostatitis intoxication) 2) 
    (= (craves achievement guava) 5) 
    (= (locale scallop oregon) 3) 
    (= (craves achievement cucumber) 2) 
    (= (craves excitement scallop) 5) 
    (= (craves abrasion beef) 3) 
 )
 (:bound 6)
 )
