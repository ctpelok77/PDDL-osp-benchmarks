(define (problem strips-mysty-x-8)
 ...
 (:utility 
    ...
    (= (craves rest muffin) 5) 
    (= (craves learning pepper) 1) 
    (= (harmony love uranus) 2) 
    (= (craves lubricity arugula) 3) 
    (= (fears sciatica aesthetics) 4) 
    (= (craves abrasion rice) 4) 
    (= (craves anxiety pork) 2) 
    (= (harmony rest uranus) 3) 
    (= (craves anger muffin) 3) 
    (= (craves sciatica baguette) 5) 
    (= (locale tuna bosnia) 5) 
    (= (craves love flounder) 1) 
    (= (fears hangover rest) 2) 
    (= (locale tofu goias) 4) 
 )
 (:bound 16)
 )
